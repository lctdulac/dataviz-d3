...
function initialize_graph_and_scales(chosen_year) {
    ...
    d3.csv(path, (data) => {
        ...
        // data_2014 = data.filter(function(d) { return d.year == chosen_year; })
        ...
        maingroup.append("text")
            ...
            //.text("Total Consumption this year - " + numberWithCommas(d3.sum(data, function(d) { if (d.year == chosen_year) {return d.cons; }})) + " MWh");


        colorScale = d3.scaleLinear() // !
            .domain([0, d3.max(data, function(d) { if (d.year == chosen_year) {return d.cons; }}) / 2, d3.max(data, function(d) { if (d.year == chosen_year) {return d.cons; }})])
        ...
        heatMap = maingroup.selectAll(".day")
            ...
            //     if (d.year == chosen_year) {
            //         return colorScale(d.cons);
            //     }
            // })

        updateGrid2(chosen_year);


        consoScale = d3.scaleLinear() // !
            .domain([0, d3.max(data, function(d) { if (d.year == chosen_year) {return d.cons; }})])
            .range([0, width])

        numStops = 3;
        consoPoint = [0, d3.max(data, function(d) { if (d.year == chosen_year) {return d.cons; }}) / 2, d3.max(data, function(d) { if (d.year == chosen_year) {return d.cons; }})];
...
function updateGrid2(chosen_year) {

    console.log("changing to " + chosen_year)

    d3.selectAll("text.subtitle")
        .transition()
        .duration(50)
        .text("Total Consumption this year - " + numberWithCommas(d3.sum(heatMap.data(), function(d) {
            if (d.year == chosen_year) { return d.cons; }
        })) + " MWh");;


    d3.selectAll("rect.colored")
        .data(heatMap.data())
        .transition()
        .duration(200)
        .style("fill", function(d) {
            if (d.year == chosen_year) {
                console.log(d.year + " = " + chosen_year + " coloring...")
                return colorScale(d.cons);
            }
        })
